<template>
  <div>
   <el-button plain="true" @click="openHTML(airportNamePieA) ">空客飞机诊断单</el-button>
...
export default {
 ...
 watch:{
    airportNamePieA:function(newValue, oldValue){
...
methods:{
      ...
      openHTML(airportNamePieA) {

        var detail_A = require("./data/一月份机场数据_Aaddreason.json");
        for(var i = 0;i<detail_A.length;i++){
            var temp= detail_A[i];
            var data= (temp["B"]);
          //alert(data);
          if(data==airportNamePieA.toString()){
          ...
          if(data==airport.toString()){
            //alert("good");
              ...
              //     Adata[j] = parseInt((temp[date]));
               //alert(Adata);
               Adata[0][1] = parseInt(temp["fengli1"]);
               Adata[1][1]= parseInt(temp["fengli2"]);
               Adata[2][1]= parseInt(temp["fengli3"]);
               Adata[3][1] = parseInt(temp["fengli4"]);
               Adata[4][1]= parseInt(temp["fengli5"]);
               Adata[5][1] = parseInt(temp["fengli6"]);
              }
